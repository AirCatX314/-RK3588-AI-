...
class LabClient(QMainWindow):
    ...
    def create_monitor_page(self):
        ...
        self.call_status_label.setStyleSheet("font-size: 15px; color: #ffc107; font-weight: bold;")
    ...
    def create_agent_page(self):
        ...
        page.setStyleSheet("""
            ...
            QPushButton:hover { background: #373e55; }
            QPushButton:checked {
                background: rgba(47, 128, 255, 0.20);
                border: 1px solid rgba(47, 128, 255, 0.70);
                color: #d8e7ff;
            }
            ...
            QLineEdit:focus, QComboBox:focus { border-color: rgba(47, 128, 255, 0.85); }
        ...
        subtitle.setStyleSheet("font-size: 12px; color: #aab2c5;")
        ...
        self.agent_risk_label.setStyleSheet("padding: 6px 10px; border-radius: 13px; background: rgba(64, 201, 121, 0.14); color: #40c979; font-weight: 800;")
        header_layout.addWidget(self.agent_risk_label)

        self.agent_model_label = QLabel("--")
        self.agent_model_label.setStyleSheet("padding: 6px 10px; border-radius: 13px; background: rgba(255, 255, 255, 0.08); color: #dce3f2;")
        header_layout.addWidget(self.agent_model_label)

        self.agent_service_label = QLabel("--")
        self.agent_service_label.setStyleSheet("color: #aab2c5;")
        ...
        self.agent_reason_label.setStyleSheet("color: #aab2c5; padding: 0 4px;")
        ...
        self.agent_upload_label.setStyleSheet("color: #aab2c5; font-size: 12px;")
        ...
        self.agent_model_hint.setStyleSheet("color: #aab2c5; font-size: 12px;")
        ...
        self._add_agent_chat_item("Agent", "你好，我是 LabSafe Agent。你可以直接提问。", "#f7f8fb")
    ...
    def create_settings_page(self):
        ...
        self.detection_btn.setStyleSheet("color: #fff; font-size: 14px;")
    ...
    def _apply_agent_status(self, data):
        ...
        bg = {
            'normal': 'rgba(64, 201, 121, 0.14)',
            'notice': 'rgba(34, 211, 238, 0.14)',
            'warning': 'rgba(242, 184, 75, 0.16)',
            'danger': 'rgba(239, 68, 68, 0.18)',
        }.get(risk, 'rgba(255, 255, 255, 0.08)')
        if hasattr(self, 'agent_risk_label'):
            self.agent_risk_label.setText(risk)
            self.agent_risk_label.setStyleSheet(f"padding: 6px 10px; border-radius: 13px; background: {bg}; color: {color}; font-weight: 800;")
    ...
    def _render_agent_actions(self, actions):
        ...
        if not actions:
            ...
            label.setStyleSheet("color: #aab2c5;")
        ...
        for action in actions:
            ...
            title.setStyleSheet("color: #ffb4b4; font-weight: bold;")
            box_layout.addWidget(title)
            reason = QLabel(action.get('reason', ''))
            reason.setWordWrap(True)
            reason.setStyleSheet("color: #aab2c5; font-size: 12px;")
            box_layout.addWidget(reason)
            token = action.get('confirmation_token') or action.get('token')
            btn = QPushButton("确认执行")
            btn.setStyleSheet("QPushButton { background: #ef4444; color: #fff; border-color: #ef4444; font-weight: bold; } QPushButton:hover { background: #dc2626; }")
    ...
    def send_agent_message(self):
        ...
        self.agent_pending_uploads = []
        self._refresh_agent_upload_label()
        self._add_agent_chat_item("我", content, "#dce6f4", upload_snapshot)
    ...
    def _post_agent_chat(self, content, deep_thinking=False, web_search=False, attachment_ids=None):
        try:
            resp = requests.post(
                ...
                timeout=45 if (deep_thinking or web_search) else 22
            )
            data = resp.json()
        except Exception as e:
    ...
    def _apply_agent_chat_response(self, data):
        ...
        if used_thinking_label:
            ...
            self._add_agent_chat_item("Agent", reply or "Agent 暂无回复", "#f7f8fb", data.get('attachments') if isinstance(data, dict) else [])
        if used_thinking_label and isinstance(data, dict) and data.get('attachments'):
            ...
            self._add_agent_chat_item("Agent", "附件", "#aab2c5", data.get('attachments') or [])
    ...
    def _add_agent_chat_item(self, sender, content, color, attachments=None):
        if hasattr(self, 'agent_chat_layout'):
            ...
            role.setStyleSheet("color: #f7f8fb; font-weight: 800;")
            ...
            if is_user:
                ...
                text.setStyleSheet(f"color: {color or '#f4f6fb'}; line-height: 1.45;")
            ...
            if att.get('type') == 'image':
                try:
                    ...
                    if resp.ok and pix.loadFromData(resp.content):
                        ...
                        label.setStyleSheet("border: 1px solid rgba(255,255,255,0.12); border-radius: 8px; padding: 4px;")
                        layout.addWidget(label)
                except Exception:
                    pass
            link = QLabel(f'<a href="{full_url}">{title}</a>')
            link.setOpenExternalLinks(True)
            link.setStyleSheet("color: #8fbaff; font-size: 12px;")
    ...
    def _start_agent_thinking(self, text):
        ...
        self.agent_thinking_step = 0
        self.agent_thinking_label = self._add_agent_chat_item("Agent", text + "...", "#dce6f4")
    ...
    def _apply_agent_action_result(self, data):
        ...
        self._add_agent_chat_item("系统", message, "#ffc66d")
        self.show_toast(message)
    ...
    def on_connected(self):
        ...
        self.status_label.setStyleSheet("color: #4caf50; font-size: 12px; padding: 12px 15px; border-top: 1px solid rgba(255,255,255,0.05);")
        self.add_message("系统", "已连接到服务器", "system")
    
    def on_disconnected(self):
        self.status_label.setText("● 未连接")
        self.status_label.setStyleSheet("color: #f44336; font-size: 12px; padding: 12px 15px; border-top: 1px solid rgba(255,255,255,0.05);")
    ...
    def send_fire_alert(self):
        ...
        self.ws_thread.send_message(msg)
        self.add_message("我", "🔥 火灾报警已发送", "danger")
    ...
    def _apply_emergency_call_status(self, data):
        ...
        if hasattr(self, 'call_status_label'):
            color = '#4caf50' if state == 'idle' else '#ff2d2d' if state in ('in_call', 'dialing', 'ringing') else '#ffc107'
            if state in ('sim_missing', 'error', 'port_missing', 'sim_pin', 'sim_puk'):
                color = '#ff9800'
            self.call_status_label.setText(display_text)
            self.call_status_label.setStyleSheet(f"font-size: 15px; color: {color}; font-weight: bold;")
    ...
    def _apply_fire_alarm_ui(self, active, reason=''):
        ...
        if hasattr(self, 'fire_label'):
            if active:
                ...
                self.fire_label.setStyleSheet("font-size: 16px; color: #ff2d2d; font-weight: bold;")
            else:
        ...
        if hasattr(self, 'sys_label'):
            if active:
                ...
                self.sys_label.setStyleSheet("font-size: 16px; color: #ff2d2d; font-weight: bold;")
            else:
    ...
    def _apply_environment_status(self, data):
        ...
        if hasattr(self, 'humidity_label'):
            if humidity is None:
                ...
                self.humidity_label.setStyleSheet("font-size: 16px; color: #ffc107; font-weight: bold;")
            else:
                self.humidity_label.setText(f"{float(humidity):.1f}%")
                color = "#03a9f4" if sensor_status == "ok" else "#ffc107"
                self.humidity_label.setStyleSheet(f"font-size: 16px; color: {color}; font-weight: bold;")
